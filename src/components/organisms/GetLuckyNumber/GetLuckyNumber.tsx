...
export function GetLuckyNumber() {
    ...
    const [luckyNumber, setLuckyNumber] = useState<number | null>(() => {
        const cookieValue = cookiesUtils.getCookie(APP_CONSTANTS.COOKIE_NUMBER_NAME);
    ...
    // Precarrega imagens não críticas que serão usadas após a ação do usuário
    useEffect(() => {
        const preload = (src: string) => {
            const img = new Image();
            // Sugere decodificação assíncrona para não bloquear o thread principal
            // e deixa o navegador otimizar o momento do decode
            (img as HTMLImageElement & { decoding?: string }).decoding = "async";
            img.src = src;
        };

        preload(imageRoletaFundo);
        preload(imageRoletaFrente);

        // Sinaliza alta prioridade para o botão principal ainda antes de renderizar a imagem
        const link = document.createElement("link");
        link.rel = "preload";
        link.as = "image";
        link.setAttribute("fetchpriority", "high");
        link.href = imageButton;
    ...
    //         setLoadingNumber(Math.floor(Math.random() * 999) + 1);
    //     }, 50);
    ...
    const fetchLuckyNumber = async () => {
        ...
        const spinInterval = setInterval(() => {
            setLoadingNumber(Math.floor(Math.random() * 101));
        }, 50);

        try {
            const result = await drawService.getDrawNumber();

            if (result.success) {
                const number = result.data.number;
                cookiesUtils.setCookie(APP_CONSTANTS.COOKIE_NUMBER_NAME, String(number), 7);
                setLuckyNumber(number);
                // setApiMessage(result.message ?? null);
            } else {
                // setLuckyNumber(null);
                setApiMessage(result.message ?? "Não foi possível obter o número agora.");
            }
        } catch (error: unknown) {
            // resposta do servidor quando todos os números já foram usados (codigo 400)
            // {
            //     "success": false,
            //     "data": null,
            ...
            if (error instanceof AxiosError) {
                if (error.response?.status === 400 && error.response?.data.message === "Todos os números já foram sorteados!") {
                    setIsAllNumbersUsed(true);
                    setApiMessage("Todos os números já foram sorteados!");
                } else {
                    setApiMessage("Não foi possível obter o número agora.");
                }
            } else {
                setApiMessage("Houve um erro ao obter o número agora.");
            }
        } finally {
            setTimeout(() => {
                clearInterval(spinInterval);
    ...
    return (
        <div className="flex-1 px-6 py-10 pt-6 text-center">
            ...
            {luckyNumber === null && !isSpinning ? (
                ...
                <div className="mt-8">
                    {isAllNumbersUsed ? (
                        <div className="space-y-8 animate__animated animate__fadeIn animate__slower">
                            <p className="text-2xl font-bold">Números Esgotados!</p>

                            <p className="text-xl">Todos os números do sorteio já foram distribuídos.</p>
                        ...
                        <div className="space-y-8 animate__animated animate__fadeInUp">
                            <p className="text-xl">
                                Temos 100 números disponíveis. <br />
                                Clique no <b>botão abaixo</b> e garanta o seu!
                            </p>

                            <Button
                                onClick={fetchLuckyNumber}
                                disabled={isSpinning || apiMessage !== null}
                                aria-busy={isSpinning || apiMessage !== null}
                                aria-label="Sortear número da sorte"
                                type="button"
                                className="flex justify-center items-start bg-transparent hover:bg-transparent p-0 w-full cursor-pointe"
                            >
                                <img
                                    src={imageButton}
                                    alt="Aperte aqui"
                            ...
                            {apiMessage && (
                                <div
                                    role="alert"
                                    className="mt-4 inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-background/60 dark:bg-background/40 backdrop-blur px-3 py-1.5 shadow-sm"
                                >
                                    <AlertCircle className="size-4 text-destructive" aria-hidden="true" />
                                    <p className="text-sm translate-y-0.5">{apiMessage}</p>
                                    ...
                                    <Button
                                        ...
                                        className="size-7 rounded-full -mr-1"
                ...
                <div className="flex flex-col items-center gap-18 animate__animated animate__fadeIn">
                    <div className="flex flex-col items-center gap-3">
                        <div className={`relative w-full max-w-[265px] animate__animated ${!isSpinning ? 'animate__bounceIn' : ''}`}>
                            <img
                                src={imageRoletaFundo}
                                alt="Roleta"
                            ...
                            {isSpinning ? (
                                <p className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-druk text-8xl text-black/80 pt-1.5 transition-all duration-100 ${isSpinning ? 'animate-pulse' : ''}`}>
                                    {formatNumber(loadingNumber ?? 0)}
                                </p>
                            ) : (
                                <p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-druk text-8xl text-black/80 pt-1.5">
                                    {formatNumber(luckyNumber ?? 0)}
                                </p>
                            )}

                            <img
                                src={imageRoletaFrente}
                                alt="Roleta"
                        ...
                        <div className="flex flex-col items-center gap-2">
                            {isSpinning ? (
                                <>
                                <p className="text-xl">
                                    Descobrindo seu <span className="font-bold">número da sorte</span>...
                                </p>
                                ...
                                <p className="text-xl animate__animated animate__fadeIn">
                                    Este é seu <span className="font-bold">número da sorte</span>
                                </p>
                                <IconTrevo className="w-14 h-14 text-primary animate__animated animate__fadeIn" />
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    {!isSpinning && (
                        <p className="text-xl font-bold animate__animated animate__fadeIn">
                            Bom evento!
                        </p>
                    )}
                </div>
